...
void menu()
{
  ...
  printf("3 - Ordenar alunos por matrícula\n");
  printf("4 - Sair\n");
}
...
void ordernarPorMatricula(int quantidade, estrut_aluno alunos[])
{
  int i, j;
  for (i = 0; i < quantidade; i++)
  {
    int indexMenorMatricula = i;
    for (j = i + 1; j < quantidade; j++)
    {
      if (alunos[j].matricula < alunos[indexMenorMatricula].matricula)
      {
        indexMenorMatricula = j;
      }
    }

    estrut_aluno alunoTemp = alunos[i];
    alunos[i] = alunos[indexMenorMatricula];
    alunos[indexMenorMatricula] = alunoTemp;
  }
}
...
int main()
{
  ...
  while (opcao != 4)
  {
    ...
    case 3:
      printf("Ordenar alunos por matrículas:\n");
      ordernarPorMatricula(quantidade, alunos);
      break;
